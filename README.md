...
- **Visual diagrams on the foundational lessons**: the first stripe of each belt gets a concrete picture, not just text — a dot array clustered into equal groups for multiplication/division, a hop-by-hop number line for addition/subtraction, a shaded bar for fractions/decimals, a labeled "variable box" beside unit squares for algebra, a level balance scale for equations, an input/rule/output "function machine" and a rise-over-run staircase for functions — since the first lesson of anything is the one that has to land hardest. Some diagrams appear on every problem in a drill, not just the lesson, when reading the picture *is* the skill (identifying a shaded fraction).
...
Coral Belt and Black Belt's remaining degree (Pre-Calculus) are on the map as "coming soon" — same engine, new content to be filled in degree by degree.
