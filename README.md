...
- **Visual diagrams on the foundational lessons**: the first stripe of each belt gets a concrete picture, not just text — a dot array clustered into equal groups for multiplication/division, a hop-by-hop number line for addition/subtraction, a shaded bar for fractions/decimals, a labeled "variable box" beside unit squares for algebra, a level balance scale for equations, an input/rule/output "function machine" and a rise-over-run staircase for functions, a 2×2 area grid (the "box method") for factoring in Pre-Calculus — since the first lesson of anything is the one that has to land hardest. Some diagrams appear on every problem in a drill, not just the lesson, when reading the picture *is* the skill (identifying a shaded fraction).
...
- **Black Belt · Degree 4 (Equations)** — 10 stripes: what an equation means (a level balance scale — x plus known units on one pan, known units on the other), one-step equations (undo addition/subtraction, then multiplication/division), two-step equations, equations with negative solutions, variables on both sides, equations with distribution, combining like terms first, distribution with variables on both sides, and a multi-step capstone that distributes, simplifies, and gathers x-terms from both sides. Solving for x is always a single answer — the drill and lesson both display it as "x = —", never concatenated after the equation itself (which already contains its own "=").
- **Black Belt · Degree 5 (Functions)** — 10 stripes: what a function means (an input/rule/output "function machine"), function notation (evaluating f(x) for positive, then negative, inputs), what slope means (a rise-over-run staircase, answered as a real fraction), calculating slope from two labeled points, finding the y-intercept (the constant term, no calculation needed once you see the pattern), finding the x-intercept (solving f(x) = 0 — a direct callback to Equations), what a quadratic function means (a handful of plotted points tracing a parabola's U-shape), evaluating quadratic functions, and a capstone that evaluates a linear and a quadratic function at the same input and adds the results.
- **Black Belt · Degree 6 (Pre-Calculus)** — 10 stripes, the final Black Belt degree: what factoring means (a 2×2 area-grid "box method" for expanding (x+a)(x+b)), factoring trinomials by finding a matching pair of numbers, solving quadratic equations by factoring, the quadratic formula (called "a fórmula de Bhaskara" in Portuguese, matching how it's actually taught in Brazil), completing the square, the discriminant, simplifying square roots, the laws of exponents, systems of linear equations solved by elimination, and a capstone that rearranges an equation into standard form before factoring and solving it. Introduces two new answer formats — "pair" (two independently meaningful numbers, like a quadratic's two roots or a system's x/y solution) and "radical" (a coefficient outside a simplified square root) — both reusing the existing two-box side-by-side UI with just a new separator character. **Black Belt is now complete** — all 6 degrees, 60 stripes.

Coral Belt (Calculus I) is on the map as "coming soon" — same engine, new content to be filled in.
